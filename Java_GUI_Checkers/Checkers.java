 ...
 * BLUE always starts the game.  If a player can jump an opponent's
...
public class Checkers extends JPanel {
   ...
   public static void main(String[] args) {
      ...
      ImageIcon img = new ImageIcon("checkers_PNG34.png");
      ...
      String filepath = "Backround_Music.wav";
      
      Checkers musicObject = new Checkers();
   ...
   public Checkers() {
      ...
      setBackground(new Color(19,27,29));  // Basic Black background.
   ...
   private class Board extends JPanel implements ActionListener, MouseListener {
      ...
      int currentPlayer;      // Whose turn is it now?  The possible values
                              //    are CheckersData.BLUE and CheckersData.BLACK.
      ...
      void doNewGame() {
         ...
         currentPlayer = CheckersData.BLUE;   // BLUE moves first.
         legalMoves = board.getLegalMoves(CheckersData.BLUE);  // Get BLUE's legal moves.
         selectedRow = -1;   // BLUE has not yet selected a piece to move.
      ...
      void doResign() {
         ...
         if (currentPlayer == CheckersData.BLUE)
            gameOver("BLUE resigns.  BLACK wins.");
         else
            gameOver("BLACK resigns.  BLUE wins.");
      }
      ...
      void doMakeMove(CheckersMove move) {
         ...
         if (currentPlayer == CheckersData.BLUE) {
            ...
            if (legalMoves == null){
               gameOver("BLACK has no moves.  BLUE wins.");
               JOptionPane.showMessageDialog(frame,"BLACK has no moves.  BLUE wins.");
            }
            ...
            legalMoves = board.getLegalMoves(currentPlayer);
            if (legalMoves == null){
               gameOver("BLUE has no moves.  BLACK wins.");
               JOptionPane.showMessageDialog(frame,"BLUE has no moves.  BLACK wins.");
            }
            else if (legalMoves[0].isJump())
         ...
         /* Make sure the board is BLUErawn in its new state. */
    ...
    * Note that BLUE moves "up" the board (i.e. row number decreases)
   ...
   private static class CheckersData {
      ...
      /*  The following constants represent the possible contents of a square
          on the board.  The constants BLUE and BLACK also represent players
          in the game. */
       ...
       * and all such squares in the last three rows contain BLUE squares.
       */
      void setUpGame() {
         for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
      ...
      void makeMove(int fromRow, int fromCol, int toRow, int toCol) {
         ...
         if (toRow == 0 && board[toRow][toCol] == BLUE)
       ...
       * should be one of the constants BLUE or BLACK; if not, null
      ...
      CheckersMove[] getLegalMoves(int player) {
         ...
         ArrayList<CheckersMove> moves = new ArrayList<CheckersMove>();  // Moves will be stoBLUE in this list.
      ...
      CheckersMove[] getLegalJumpsFrom(int player, int row, int col) {
         ...
         if (player == BLUE)
            ...
            playerKing = BLACK_KING;
         ArrayList<CheckersMove> moves = new ArrayList<CheckersMove>();  // The legal jumps will be stoBLUE in this list.
         if (board[row][col] == player || board[row][col] == playerKing) {
            if (canJump(player, row, col, row+1, col+1, row+2, col+2))
               moves.add(new CheckersMove(row, col, row+2, col+2));
            if (canJump(player, row, col, row-1, col+1, row-2, col+2))
      ...
      private boolean canJump(int player, int r1, int c1, int r2, int c2, int r3, int c3) {
         ...
         if (player == BLUE) {
            ...
            if (board[r2][c2] != BLUE && board[r2][c2] != BLUE_KING)
               return false;  // There is no BLUE piece to jump.
            return true;  // The jump is legal.
         }
